...
def test_auto_backup_functionality():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if len(remaining_backups) <= 10:
            ...
            print(
                f"❌ Cleanup failed - {len(remaining_backups)} backups remain (should be ≤10)"
            )
            return False
...
def test_backup_cleanup_edge_cases():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if len(remaining_backups) == 5:
            ...
            print(
                f"❌ Custom keep count failed - {len(remaining_backups)} backups remain"
            )
            return False
...
def test_backup_file_integrity():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        test_config = {
            ...
            "exact_matches": {"alias1": "company-1", "alias2": "company-2"},
            "partial_matches": {"partial1": "company-1", "partial2": "company-3"},
