...
try:
    ...
    logging.warning(
        f"Some dependencies are missing: {e}. OCR functionality will be limited."
    )
...
class DateExtractor:
    ...
    def extract_date_from_text(text: str) -> Optional[str]:
        ...
        date_keywords = [
            ...
            "juillet",
            "relevé",
            "releve",  # Add 'relevé' as it appears in the text
        ]
        ...
        if not candidates:
            for line in search_lines:
                ...
                for pattern in standalone_patterns:
                    ...
                    for match in matches:
                        logging.debug(
                            f"Found standalone robust date pattern match: {match} with pattern: {pattern}"
                        )
                        try:
                            parsed_date = DateExtractor._parse_robust_date_match(
                                match, pattern
                            ...
                            if parsed_date:
                                candidates.append(
                                    (
                                        parsed_date,
                                        f"Standalone robust date: {line.strip()}",
                                        12,
                                    )
                                )
                        except Exception as e:
                            logging.debug(
                                f"Failed to parse standalone robust date match {match}: {e}"
                            )
                            continue
                ...
                for pattern in year_patterns:
                    ...
                    for match in matches:
                        ...
                        try:
                            ...
                            corrected_year = (
                                DateExtractor._apply_ocr_correction_to_year(match)
                            )
                            if corrected_year and 1900 <= corrected_year <= 2100:
                                # Try to find month and day in the same line
                                month_day = DateExtractor._extract_month_day_from_line(
                                    line
                                )
                                if month_day:
                                    month, day = month_day
                                    parsed_date = (
                                        f"{corrected_year:04d}-{month:02d}-{day:02d}"
                                    )
                                    candidates.append(
                                        (
                                            parsed_date,
                                            f"OCR-corrected year: {line.strip()}",
                                            8,
                                        )
                                    )
                        except Exception as e:
                            logging.debug(
                                f"Failed to process potential year {match}: {e}"
                            )
                            continue
        ...
        try:
            if len(match) == 3:
                ...
                if pattern.startswith(
                    r"(\d{3}[0-9Oo])"
                ):  # YYYY-MM-DD format with OCR tolerance
                    ...
                    "relevé" in pattern.lower()
                ):  # Pattern for "Date du relevé : 7 fév 202S"
                    day = int(part1_corrected)
                    month = DateExtractor._parse_month(part2)
                    year = int(part3_corrected)
...
class InvoiceDatabase:
    ...
    def find_company_match(self, text: str) -> Optional[Tuple[str, float, str]]:
        ...
        # Try fuzzy match
        best_match = FuzzyMatcher.fuzzy_match(
            text, self.database["company_names"], threshold=0.3
        )
        if best_match:
...
class InvoiceOCRParser:
    ...
    def __init__(
        ...
        if not DEPENDENCIES_AVAILABLE:
            raise ImportError(
                "Required dependencies are not available. Please install: opencv-python, numpy, pandas, pillow, pytesseract, pdf2image, pdfplumber, PyPDF2"
            )
        ...
        for i, image in enumerate(images):
            try:
                text = pytesseract.image_to_string(
                    image, config="--oem 3 --psm 6", lang="eng+fra"  # English + French
                )
    ...
    def extract_company_name(self, text: str, expected_company: str = None) -> str:
        ...
        if not candidates:
            for company in known_companies:
                if company.lower() in text.lower():
                    candidates.append(
                        (company, f"Found in text: {company}", 15)
                    )  # Highest priority
                    break
        ...
        if not candidates:
            if (
                "compte de taxes scolaire" in text.lower()
                or "centre de services scolaire" in text.lower()
            ):
                candidates.append(
                    ("compte de taxes scolaire", "Found tax school account", 15)
                )
                ...
                if len(line_clean) > 5:
                    ...
                    if business_match:
                        ...
                        priority = (
                            ...
                            else 12 if match_type == "partial_match" else 10
                        )
                        candidates.append(
                            (
                                business_name,
                                f"Business alias {match_type}: {line_clean}",
                                priority,
                            )
                        )
                        logging.info(
                            f"Business alias found: '{line_clean}' -> '{business_name}' (type: {match_type}, confidence: {confidence})"
                        )
                        break
        ...
        if candidates:
            ...
            logging.info(
                f"Found company name: '{best_company}' from line: {source_line}"
            )
            return best_company
    ...
    def extract_invoice_total(
        self, text: str, expected_total: float = None
    ) -> Optional[str]:
        """
        ...
        OCR_CORRECTIONS = {
            ...
            "t": "7",  # T/t often misread as 7
            "D": "0",
            "d": "0",  # D/d sometimes misread as 0
            "Q": "0",
            "q": "0",  # Q/q sometimes misread as 0
            "U": "0",
            "u": "0",  # U/u sometimes misread as 0
            "C": "0",
            "c": "0",  # C/c sometimes misread as 0
            "P": "9",
            "p": "9",  # P/p sometimes misread as 9
            "F": "7",
            "f": "7",  # F/f sometimes misread as 7
            "H": "4",
            "h": "4",  # H/h sometimes misread as 4
            "K": "6",
            "k": "6",  # K/k sometimes misread as 6
            "M": "1",
            "m": "1",  # M/m sometimes misread as 1
            "N": "1",
            "n": "1",  # N/n sometimes misread as 1
            "R": "2",
            "r": "2",  # R/r sometimes misread as 2
            "V": "7",
            "v": "7",  # V/v sometimes misread as 7
            "W": "7",
            "w": "7",  # W/w sometimes misread as 7
            "X": "7",
            "x": "7",  # X/x sometimes misread as 7
            "Y": "7",
            "y": "7",  # Y/y sometimes misread as 7
        }
        ...
        def extract_amount_from_match(match: str, context: str = "") -> Optional[float]:
            ...
            try:
                ...
                logging.debug(
                    f"Standard normalization failed for '{match}', trying aggressive correction"
                )
                ...
                for wrong, right in aggressive_corrections.items():
                    if wrong in aggressive_match:
                        ...
                        logging.debug(
                            f"Aggressive OCR correction: '{match}' -> '{aggressive_match}'"
                        )
        ...
        def calculate_priority(
            amount: float, pattern_type: str, line_context: str, line_position: int
        ) -> int:
            ...
            if any(
                ...
                for keyword in ["total", "montant", "somme", "dû", "payer", "balance"]
            ):
                priority += 5
        ...
        for line_num, line in enumerate(lines):
            ...
            if (
                ...
                and "nouveau solde" in line_original.lower()
                and not line_original.endswith("CR")
            ):
                next_line = lines[line_num + 1].strip()
                if next_line == "CR" or len(next_line) < 5:
                    combined_line = line_original + " " + next_line
                    logging.debug(
                        f"Combined lines {line_num} and {line_num + 1}: '{combined_line}'"
                    )
                ...
                for match in matches:
                    ...
                    if amount:
                        priority = calculate_priority(
                            amount, "credit_card_total", combined_line, line_num
                        )
                        candidates.append(
                            (amount, f"Credit card pattern: {combined_line}", priority)
                        )
                        logging.debug(
                            f"Added candidate: {amount} from pattern '{pattern}' in line: '{combined_line}'"
                        )
            ...
            for line_num, line in enumerate(lines):
                ...
                for pattern in high_priority_patterns:
                    ...
                    for match in matches:
                        ...
                        if amount:
                            priority = calculate_priority(
                                amount, "high_priority", line_original, line_num
                            )
                            candidates.append(
                                (
                                    amount,
                                    f"High-priority pattern: {line_original}",
                                    priority,
                                )
                            )
            ...
            for line_num, line in enumerate(lines):
                ...
                for pattern in currency_patterns:
                    ...
                    for match in matches:
                        ...
                        if amount:
                            priority = calculate_priority(
                                amount, "currency", line_original, line_num
                            )
                            candidates.append(
                                (amount, f"Currency pattern: {line_original}", priority)
                            )
            ...
            for line_num, line in enumerate(lines):
                ...
                for pattern in balance_patterns:
                    ...
                    for match in matches:
                        ...
                        if amount:
                            priority = calculate_priority(
                                amount, "balance", line_original, line_num
                            )
                            candidates.append(
                                (amount, f"Balance pattern: {line_original}", priority)
                            )
            ...
            for line_num, line in enumerate(lines):
                ...
                for pattern in amount_patterns:
                    ...
                    for match in matches:
                        ...
                        if amount:
                            priority = calculate_priority(
                                amount, "amount", line_original, line_num
                            )
                            candidates.append(
                                (amount, f"Amount pattern: {line_original}", priority)
                            )
            ...
            for line_num, line in enumerate(lines):
                ...
                for pattern in general_patterns:
                    ...
                    for match in matches:
                        ...
                        if amount:
                            priority = calculate_priority(
                                amount, "general", line_original, line_num
                            )
                            candidates.append(
                                (amount, f"General pattern: {line_original}", priority)
                            )
            ...
            if all_amounts:
                ...
                for amount, line in all_amounts:
                    if 10 <= amount <= 10000:  # Reasonable range for credit card bills
                        ...
                        candidates.append(
                            (amount, f"Largest reasonable amount: {line}", priority)
                        )
                        break
        ...
        if candidates:
            ...
            logging.info(
                f"Found invoice total: '{formatted_amount}' from line: {source_line} (priority: {priority})"
            )
    ...
    def calculate_confidence(
        self, company: str, total: Optional[float], date: Optional[str], text: str
    ) -> str:
    ...
    def parse_invoice(
        ...
        try:
            ...
            if invoice_date:
                ...
                if self.debug:
                    logging.debug(
                        f"Text processed for date extraction (first 500 chars): {text[:500]}"
                    )

            # Calculate confidence
            confidence = self.calculate_confidence(
                company_name, invoice_total, invoice_date, text
            )
            ...
            if auto_rename and confidence != "low":
                new_file_path = self.rename_pdf(
                    pdf_path, company_name, invoice_total, invoice_date
                )
    ...
    def parse_invoices_batch(
        self, folder_path: str, output_file: str = None, auto_rename: bool = False
    ):
    ...
    def rename_pdf(
        self, pdf_path: str, company_name: str, invoice_total: str, invoice_date: str
    ) -> str:
        ...
        try:
            ...
            clean_date = (
                invoice_date if invoice_date else "nodate"
            )  # Keep YYYY-MM-DD format
            clean_company = (
                self._clean_filename(company_name)
            ...
            if (
                clean_date != "nodate"
                and clean_company != "unknown"
                and clean_total != "nototal"
            ):
...
class CreditCardBillParser:
    ...
    def extract_text_with_ocr(self, images: List) -> str:
        ...
        for i, image in enumerate(images):
            try:
                ...
                text = pytesseract.image_to_string(
                    image, config=custom_config, lang="eng+fra"  # English + French
                )
    ...
    def extract_credit_card_total(
            ...
            for line_num, line in enumerate(lines):
                ...
                for pattern in payment_due_patterns:
                    ...
                    for match in matches:
                        ...
                        if amount:
                            priority = calculate_priority(
                                amount, "payment_due", line_original, line_num
                            )
                            candidates.append(
                                (
                                    amount,
                                    f"Payment due pattern: {line_original}",
                                    priority,
                                )
                            )
        ...
        if candidates:
            ...
            logging.info(
                f"Found credit card total: '{formatted_amount}' from line: {source_line} (priority: {priority})"
            )
            return formatted_amount
    ...
    def parse_credit_card_bill(
        self, pdf_path: str, expected_total: float = None
    ) -> Dict[str, Any]:
