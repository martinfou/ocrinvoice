...
def example_ocr_correction():
    ...
    for input_text, expected in ocr_errors:
        ...
        print(
            f"Input: {input_text} -> Output: {total} -> Expected: {expected} -> {'✓' if success else '✗'}"
        )
...
def example_batch_processing():
    ...
    print(
        f"  Successful extractions: {sum(1 for r in results if r.get('invoice_total'))}"
    )
    print(
        f"  High confidence: {sum(1 for r in results if r.get('confidence') == 'high')}"
    )
    print(
        f"  Medium confidence: {sum(1 for r in results if r.get('confidence') == 'medium')}"
    )
    print(
        f"  Low confidence: {sum(1 for r in results if r.get('confidence') == 'low')}"
    )
...
def main():
    ...
    print(
        "including OCR correction, multiple format support, and priority-based selection."
    )
    print("=" * 80)
