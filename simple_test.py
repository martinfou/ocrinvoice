...
def test_ocr_correction():
    ...
    for input_text, expected in test_cases:
        ...
        try:
            ...
            print(
                f"Input: {input_text} -> Output: {formatted_amount} -> Expected: {expected} -> {'✓' if success else '✗'}"
            )
        except ValueError:
    ...
    for input_text, expected_match in test_cases:
        ...
        for pattern in credit_card_patterns:
            ...
            if matches:
                ...
                print(
                    f"Input: {input_text} -> Match: {actual_match} -> Expected: {expected_match} -> {'✓' if success else '✗'}"
                )
                break
