...
class TestSuite:
    ...
    def __init__(
        self,
        test_data_file: str = "/Volumes/T7/Dropbox/scan-snap/facture/ocr-test-data/test_data.json",
    ):
    ...
    def run_single_test(self, test_case: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Compare results
        company_match = self._normalize_company_name(
            actual_company
        ) == self._normalize_company_name(expected_company)
    ...
    def generate_report(
        ...
        output_file: str = "/Volumes/T7/Dropbox/scan-snap/facture/ocr-test-data/test_report.csv",
    ):
        ...
        both_matches = sum(
            1 for r in results if r["company_match"] and r["total_match"]
        )
        ...
        print(
            f"   Company Matches: {company_matches}/{total_tests} ({company_matches/total_tests*100:.1f}%)"
        )
        print(
            f"   Total Matches: {total_matches}/{total_tests} ({total_matches/total_tests*100:.1f}%)"
        )
        print(
            f"   Both Match: {both_matches}/{total_tests} ({both_matches/total_tests*100:.1f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--add", action="store_true", help="Add new test cases interactively"
    )
    parser.add_argument("--run", action="store_true", help="Run all test cases")
    parser.add_argument(
        "--report",
        default="/Volumes/T7/Dropbox/scan-snap/facture/ocr-test-data/test_report.csv",
    ...
    if args.add:
        ...
        test_suite.add_test_case(
            args.pdf, args.company, args.total, args.description or ""
        )
        ...
        print(
            "  python test_suite.py --pdf invoice.pdf --company 'ABC Corp' --total 123.45"
        )
        print("\n  # Run all tests:")
