...
def add_official_business_name(business_name):
    ...
    try:
        ...
        print(
            f"📋 Total official business names: {len(alias_manager.get_official_names())}"
        )
...
def main():
    ...
    match_type_internal = (
        "exact_matches" if match_type == "exact" else "partial_matches"
    )
    ...
    try:
        ...
        print(
            f"✅ Successfully added: '{alias_text}' -> '{business_name}' ({match_type} match)"
        )
