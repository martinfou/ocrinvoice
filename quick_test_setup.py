...
def scan_facture_folder(folder_path: str) -> List[Dict[str, Any]]:
    ...
    for pdf_file in pdf_files:
        ...
        if result:
            ...
            print(
                f"✅ Parsed: {pdf_file.name} -> {result['company']} (${result['total']:.2f})"
            )
        else:
...
def create_test_data_from_facture(
    folder_path: str,
    output_file: str = "/Volumes/T7/Dropbox/scan-snap/facture/ocr-test-data/auto_test_data.json",
):
...
def interactive_review_test_cases(test_data: Dict[str, Any]):
    ...
    for i, test_case in enumerate(test_data["tests"], 1):
        ...
        while True:
            action = (
                input("Action (c=confirm, e=edit, s=skip, q=quit): ").strip().lower()
            )
            ...
            if action == "c":
                ...
                new_company = input(
                    f"New company name [{test_case['expected_company']}]: "
                ).strip()
                if new_company:
                    test_case["expected_company"] = new_company

                new_total = input(
                    f"New total [{test_case['expected_total']}]: "
                ).strip()
                if new_total:
                    try:
...
def main():
    ...
    parser.add_argument(
        "--folder",
        default="/Volumes/T7/Dropbox/scan-snap/facture",
        help="Path to facture folder",
    )
    parser.add_argument(
        "--output",
        default="/Volumes/T7/Dropbox/scan-snap/facture/ocr-test-data/auto_test_data.json",
