...
class DateExtractor:
    ...
    def extract_date_from_text(text: str) -> Optional[str]:
        ...
        date_keywords = [
            ...
            'modification', 'juillet'  # Add 'juillet' as it appears in the text
        ]
            ...
            if any(keyword in line_lower for keyword in date_keywords):
                ...
                for pattern in date_patterns:
                    ...
                    for match in matches:
                        print(f"Found date pattern match: {match} with pattern: {pattern}")
                        try:
        ...
        if not candidates:
            ...
            for i, line in enumerate(search_lines):
                ...
                for pattern in date_patterns:
                    matches = re.findall(pattern, corrected_line, re.IGNORECASE)
                    for match in matches:
                        print(f"Found date pattern match (no keyword): {match} with pattern: {pattern}")
                        try:
                            parsed_date = DateExtractor._parse_date_match(match, pattern)
                            if parsed_date:
        ...
        if not candidates:
            ...
            for line in search_lines:
                ...
                for pattern in standalone_patterns:
                    ...
                    for match in matches:
                        print(f"Found standalone date pattern match: {match} with pattern: {pattern}")
                        try:
                            parsed_date = DateExtractor._parse_date_match(match, pattern)
                            if parsed_date:
