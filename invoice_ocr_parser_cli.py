...
class InvoiceDatabase:
    ...
    def find_company_match(self, text: str) -> Optional[Tuple[str, float, str]]:
        """Find a company match in the database using fuzzy matching"""
        ...
        best_match = FuzzyMatcher.fuzzy_match(
            target=text,
            candidates=self.database["company_names"],
            threshold=0.7
        )
        
        if best_match:
            confidence = self.database["companies"].get(best_match, 1)
            return (best_match, confidence, "database_fallback")
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "unique_companies": len(self.database["company_names"]),
            "most_frequent": max(self.database["companies"].items(), key=lambda x: x[1]) if self.database["companies"] else None
        }
...
class InvoiceOCRParser:
    ...
    def __init__(self, tesseract_path: Optional[str] = None, debug: bool = False, use_database: bool = False):
        ...
        if self.use_database:
            ...
            logger.info("Invoice database disabled")
        
        # Initialize business alias manager
        self.alias_manager = BusinessAliasManager()
        logger.info("Business alias manager initialized")
            ...
            for line in search_lines:
                ...
                if len(line_clean) > 5:
                    ...
                    if business_match:
                        ...
                        logger.info(f"Business alias found: '{line_clean}' -> '{business_name}' (type: {match_type}, confidence: {confidence})")
                        break
    ...
    def parse_invoice(self, pdf_path: str, expected_total: float = None, expected_company: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.use_database and self.invoice_db:
                # If company extraction failed, try database
                if not company_name and not expected_company:
                    db_match = self.invoice_db.find_company_match(text)
                    if db_match:
                ...
                if not invoice_total and not expected_total:
                    ...
                    if company_name:
                        normalized_company = self.invoice_db._normalize_company_name(company_name)
                        if normalized_company in self.invoice_db.database["company_totals"]:
                            # Use the most frequent total for this company
                            totals = self.invoice_db.database["company_totals"][normalized_company]
                    ...
                    if not invoice_total:
                        db_match = self.invoice_db.find_total_match(text)
                        if db_match:
                            db_company, db_total, db_confidence = db_match
                            if not company_name:
                                company_name = db_company
            ...
            if self.use_database and self.invoice_db and company_name and invoice_total:
                try:
                    total_float = float(invoice_total)
                    self.invoice_db.add_company(company_name, confidence)
...
def main():
    ...
    if not results_df.empty:
        ...
        print("Usage: python invoice_ocr_parser_cli.py [folder_path_or_file] [output_csv] [--no-database]")
        print("Example: python invoice_ocr_parser_cli.py /path/to/invoices results.csv")
        print("Example: python invoice_ocr_parser_cli.py /path/to/invoice.pdf results.csv")
        print("Example: python invoice_ocr_parser_cli.py /path/to/invoices --no-database")
        print("Example: python invoice_ocr_parser_cli.py  # Uses .env file or current directory")
        sys.exit(1)
