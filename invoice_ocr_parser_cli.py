...
def main():
    ...
    if os.path.isfile(input_path) and input_path.lower().endswith(".pdf"):
        ...
        if (
            dry_run
            and result["company_name"]
            and result["invoice_total"]
            and result["invoice_date"]
        ):
        ...
        if dry_run:
            ...
            results_df = parser.parse_invoices_batch(
                input_path, output_file, auto_rename=False
            )
    ...
    if not results_df.empty:
        ...
        if hasattr(parser, "invoice_db") and parser.invoice_db:
            ...
            if "company_frequencies" in stats:
                most_common = (
                    max(stats["company_frequencies"].items(), key=lambda x: x[1])
                    if stats["company_frequencies"]
                    else None
                )
                if most_common:
                    print(
                        f"   Most common company: {most_common[0]} ({most_common[1]} times)"
                    )
        ...
        available_columns = [
            col for col in columns_to_show if col in results_df.columns
        ]
        ...
        print(
            "  --no-auto-rename  Disable auto-rename (auto-rename is enabled by default)"
        )
        print(
            "  --dry-run       Perform a dry run (show what would be renamed without actually renaming)"
        )
        ...
        print(
            "  python invoice_ocr_parser_cli.py  # Uses .env file or current directory"
        )
        sys.exit(1)
